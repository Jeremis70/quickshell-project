...
Scope {
    ...
    readonly property var windowsInActiveWorkspace: {
		return SwitcherCommon.calculateWindowsInActiveWorkspace(
			HyprlandData.windowList,
			taskSwitcher.activeWorkspaceId,
			Hyprland.focusedMonitor?.id,
			HyprlandData.activeWindow?.address
		)
    }

	readonly property int windowCount: windowsInActiveWorkspace?.length ?? 0
	readonly property var selectedWindow: (selectedIndex >= 0 && selectedIndex < windowCount)
		? windowsInActiveWorkspace[selectedIndex]
		: null
	...
	function commitSelectionAndClose() {
		...
		if (pendingFocusAddress.length) focusAfterClose.restart()
	}
	...
	Timer {
		...
		repeat: false
		onTriggered: {
			if (!taskSwitcher.pendingFocusAddress.length) return
	...
	Variants {
		...
		PanelWindow {
			...
			readonly property real thumbIdealInnerW: {
				const list = taskSwitcher.windowsInActiveWorkspace
				if (!list || list.length === 0) return 240
				let total = 0
				for (let i = 0; i < list.length; i++) {
					total += win.thumbWFor(list[i])
					if (i > 0) total += win.thumbSpacing
				}
				return total
			}
			...
			readonly property var thumbLayout: {
				...
				return { byAddress: result.byKey, totalH: result.totalH }
			}
			...
			Rectangle {
				...
				Column {
					...
					Widgets.SmartIcon {
						...
						property var entry: DesktopEntries.heuristicLookup(
							currentWindow?.appId ?? currentWindow?.appid
							?? currentWindow?.class
							?? currentWindow?.initialClass
						)
						...
						state: "app-icon"
						icons: ({ "app-icon": iconKey })
					}

						Text {
							id: titleText
							text: {
								if (taskSwitcher.windowCount <= 0) return ""
					...
					Item {
						...
						Repeater {
							...
							delegate: Item {
								required property var modelData
								property var windowData: modelData
								property bool selected: (taskSwitcher.selectedWindow?.address && windowData?.address)
									? (taskSwitcher.selectedWindow.address === windowData.address)
									: false
							...
							readonly property real localY: SwitcherCommon.windowLocalY(windowData, win.monitorData)
								...
								x: (win.thumbLayout.byAddress && windowData?.address && win.thumbLayout.byAddress[windowData.address])
									? win.thumbLayout.byAddress[windowData.address].x
									: 0
								y: (win.thumbLayout.byAddress && windowData?.address && win.thumbLayout.byAddress[windowData.address])
									? win.thumbLayout.byAddress[windowData.address].y
									: 0
								width: thumbW
								height: thumbH
