...
QtObject {
	...
	function windowsInWorkspace(windowList, wsId, focusedMonitorId) {
		...
		return windows
			.filter(w => (w?.workspace?.id ?? -1) === wsId)
			.filter(w => (typeof focusedMonitorId === "number") ? (w?.monitor === focusedMonitorId) : true)
	}
	...
	function calculateActiveWorkspaces(workspaces, windowList, activeWorkspaceId, focusedMonitorId) {
		...
		for (let i = 0; i < windows.length; i++) {
			...
			if (!(wid > 0)) continue
			if (typeof focusedMonitorId === "number" && w?.monitor !== focusedMonitorId) continue
		...
		for (let i = 0; i < wsList.length; i++) {
			...
			if (hasWindows || id === activeWorkspaceId) list.push(ws)
		}
		...
		if (activeWorkspaceId > 0) {
			...
			if (idx > 0) list = list.slice(idx).concat(list.slice(0, idx))
		}
	...
	function monitorWorkspaceWidth(monitorData, monitor) {
		...
		return (monitorData.transform % 2 === 1)
			? ((monitor.height / monitor.scale) - (monitorData.reserved?.[0] ?? 0) - (monitorData.reserved?.[2] ?? 0))
			: ((monitor.width / monitor.scale) - (monitorData.reserved?.[0] ?? 0) - (monitorData.reserved?.[2] ?? 0))
	}

	function monitorWorkspaceHeight(monitorData, monitor) {
		if (!monitorData || !monitor) return 0
		return (monitorData.transform % 2 === 1)
			? ((monitor.width / monitor.scale) - (monitorData.reserved?.[1] ?? 0) - (monitorData.reserved?.[3] ?? 0))
			: ((monitor.height / monitor.scale) - (monitorData.reserved?.[1] ?? 0) - (monitorData.reserved?.[3] ?? 0))
	}
	...
	function calculateMultiRowLayout(items, maxWidth, spacing) {
		...
		function flushRow() {
			...
			for (let k = 0; k < row.length; k++) {
				const it = row[k]
				if (it.key) byKey[it.key] = { x, y }
				x += it.w
				if (k !== row.length - 1) x += spacing
			}
			y += rowMaxH + spacing
			row = []
			rowW = 0
		...
		for (let i = 0; i < items.length; i++) {
			const item = items[i]
			if (row.length && (rowW + spacing + item.w) > maxWidth) flushRow()
