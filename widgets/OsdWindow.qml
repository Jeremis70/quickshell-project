    ...
    LazyLoader {
        ...
        PanelWindow {
            ...
            function applyEnterAnchors() {
                ...
                if (osd.enterFrom === "right")
                    win.anchors.right = true;
                else
                    win.anchors.left = true;

                if (osd.enterFrom === "bottom")
                    win.anchors.bottom = true;
                else
                    win.anchors.top = true;
            }
            ...
            margins.left: (osd.enterFrom === "left") ? (visibleLeft + (hiddenH - visibleLeft) * slideT) : (osd.enterFrom === "right" ? 0 : visibleLeft)

            margins.right: (osd.enterFrom === "right") ? (visibleRight + (hiddenH - visibleRight) * slideT) : 0

            margins.top: (osd.enterFrom === "top") ? (visibleTop + (hiddenV - visibleTop) * slideT) : (osd.enterFrom === "bottom" ? 0 : visibleTop)

            margins.bottom: (osd.enterFrom === "bottom") ? (visibleBottom + (hiddenV - visibleBottom) * slideT) : 0
            ...
            Component.onCompleted: {
                ...
                if (osd.wantOpen) {
                    if (animSlide)
                        win.slide = 0.0;
                    if (animFade)
                        win.fade = osd.opacityShown;
                }
            }

            Connections {
                target: osd
                function onEnterFromChanged() {
                    win.applyEnterAnchors();
                }
            }
            ...
            MouseArea {
                ...
                onEntered: {
                    osd.hovering = true;
                    if (osd.hoverPausesAutoHide)
                        hideTimer.stop();
                }
                onExited: {
                    osd.hovering = false;
                    if (osd.hoverPausesAutoHide && osd.wantOpen)
                        hideTimer.restart();
                }
            }
        }
